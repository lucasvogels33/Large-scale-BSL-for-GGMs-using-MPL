...
#the function MPLRJ_solve takes as input:
...
MPLRJ_solve = function(data,iter_vec_thin){
  ...
  for (j in 1:len){
    ...
    sample_mplrj  = bdgraph.mpl( data = data, algorithm = "rjmcmc", iter = iter, burnin = burnin, jump = jump, save = save,cores=cores,g.start=MCMCstart,g.prior=g.prior,verbose=verbose)  
    
    #calculate new p matrix
    plinks_new = (olditer*plinks_old + iter*sample_mplrj$p_links)/(newiter)
    ...
    time_init = 0
    time_end = 0
    if (j==1){time_init = sample_mplrj$time_init}
    if (j==len){time_end = sample_mplrj$time_end}
    runtime = time_init + sample_mplrj$time_all_iterations + time_end
